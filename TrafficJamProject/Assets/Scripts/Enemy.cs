...
public class Enemy : MonoBehaviour
{
    ...
    void TryBrakeSounds()
    {
        ...
        if (car.braking && Time.time - timeSinceLastHonk > honkDelay)
        {
            ...
            AudioController.Instance.PlaySpatialSound(honkSounds[Random.Range(0, honkSounds.Length)], transform.position, Mathf.Min((1f / distFromCam) / 4, 1.5f));
        }
        if (car.braking && Time.time - timeSinceLastScreech > screechDelay)
        {
            timeSinceLastScreech = Time.time;
            AudioController.Instance.PlaySpatialSound(screechSounds[Random.Range(0, screechSounds.Length)], transform.position, Mathf.Min((1f / distFromCam) / 4, 1.5f));
        }
    }
